...
connection.on("ReceiveBotState", gameState => {
	...
	if(ma>0){
		...
		//First Growth / Things are going well, lets multiply //Maybe Redundent considering my While loop
		if(ma>0){
		...
		if(ma>0){
			...
			ns=ns+p[mct].tierResourceConstraints.stone+5;
		...
		//When 1 generation is no monger enough...
			//Look at Food / heat surplus so that the resource surplus is always constant
			//heat: 1,111
			
		//100 - 111 heat
		//100 - 83,333 food
		
		//Sent to the engine it will shift back and create 100 surplus both sides. 
		
		while(ma>0&&mrn>0&&!kill){
			
		nf=nf+Math.ceil(mp/1.2);
		nh=nh+Math.ceil(mp/0.9);
		if(nh>0){nw=nw+(Math.ceil(nh/5)*4);}
		console.log("Needs after Max Growth calc - Food:"+nf+", Wood:"+nw+", Heat:"+nh+", Stone: "+ns);
		...
		//What, still going? lets just hurt the trees (Triggers for kill active)
		
		//Worth wiping all stone here. Reason has to do with distance. 
			//Could catch enemies off guard, 
		
		nw=nw+10000000; 
		if(nw>0){cut();}
		
		//Add in Stone hoarding here... 
			//still to do, allow harvest Stone 
			//Test if I remove initial sustain function. 
			//Set a turn where i will allow population Growth, Maybe Round 160, and allow distant food harvest here (ignore kill)
		
		//What about stone starve early on
